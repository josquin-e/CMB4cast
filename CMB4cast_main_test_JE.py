...
def pissoffpython():
	...
	if (cib_delens):
		...
		f_l_cor = (np.ones(l_max - l_min + 1) * f_cor).flatten()
	c_l_u=np.genfromtxt('fiducial_lenspotentialCls.dat')
	c_l_l=np.genfromtxt('fiducial_lensedtotCls.dat') 
	ell_loc = c_l_u[0: l_max - l_min + 1, 0].flatten()
	d_l_conv = 2.0 * np.pi / ell_loc / (ell_loc + 1.0)
	c_l_ee_u = (c_l_u[0: l_max - l_min + 1, 2].flatten() * d_l_conv).flatten()
	c_l_ee_l = (c_l_l[0: l_max - l_min + 1, 2].flatten() * d_l_conv).flatten()
	c_l_bb_u = (c_l_u[0: l_max - l_min + 1, 3].flatten() * d_l_conv).flatten()
	c_l_bb_l = (c_l_l[0: l_max - l_min + 1, 3].flatten() * d_l_conv).flatten()
	c_l_pp = (c_l_u[0: l_max - l_min + 1, 5].flatten() / \
		 (ell_loc * (ell_loc + 1.0)) ** 2 * 2.0 * np.pi).flatten()
	f_sky_new = 0.75
...
############################################################################################################################################################################################################
def derivatives_computation(Cls_fid, params_dev, params_fid, information_channels, eta=1.0, exe='./camb', path2Cls=''):

	information_channels_tot = ['Tu', 'Eu', 'Bu', 'T', 'E', 'B', 'd']

	nell = len(Cls_fid[information_channels[0]+information_channels[0]]) 
	...
	for p in range( nparam ):
		...
		#subname_search ='drv_'
		subname ='drv_'
		subname_search = os.path.join(path2Cls, 'drv_' )
		subname = os.path.join(path2Cls, 'drv_' )
		...
		for q in range(len(params_fid.keys())):
			subname += '_'+str(params_fid.keys()[q])+str(params_fid[params_fid.keys()[q]])
			subname_search += '_'+str(params_fid.keys()[q])+str(params_fid[params_fid.keys()[q]])        
		...
		if fnames:
			...
			if params_dev[p] == 'A':
				...
				params_loc[params_dev[p]] = 0.005
			
				Cls_loc = python_camb.submit_camb( l_max_scalar=4100, l_max_tensor=4100, h=params_loc['h'], ombh2=params_loc['ombh2'], omch2=params_loc['omch2'], omnuh2=params_loc['omnuh2'], omk=params_loc['omk'], YHe=params_loc['YHe'], Neff=params_loc['Neff'], w=params_loc['w'], wa=params_loc['wa'],  tau=params_loc['tau'],As=params_loc['As'], ns=params_loc['ns'], alphas=params_loc['alphas'], nT=params_loc['nT'], r=params_loc['r'], k_scalar=params_fid['k_scalar'] , k_tensor=params_fid['k_tensor'], eta=1.0, exe=exe)
				...
				continue
				#########################################################################
			elif params_dev[p] == 'omnuh2':
				print ' taking larger steps for neutrinos '
...
def core_function(configurations, components_v, camb, \
	...
	for exp1 in experiments:
		...
		for exp2 in experiments[ind:]:
			...
			# comp sep !
			# Cls_fid_r = derivatives_computation( Cls_fid, ['r'], params_fid, information_channels, exe=camb, path2Cls = path2Cls)['r']
			Cls_fid_r = load_obj('./', '/Users/josquin1/Documents/Dropbox/self_consistent_forecast/codes/drv_r_T_E_B_d_A0.1_tau0.066_A_fgs_res1.0_Neff3.046_k_scalar0.05_wa0.0_alphas0.0_omk0.0_h67.74_b-0.8_omnuh20.0006451439_YHe0.2453_As2.142e-09_r0.001_omch20.1188_b_fgs_res-2.0_w-1.0_k_tensor0.002_ns0.9667_nT-0.000125_ombh20.0223.pkl')['r']
