...
def Fisher_computation(Cov_inv, dCldp, params_dev, fsky, information_channels, ell_min, ell_max, Cls_fid, params_fid, Cov, DESI=False, DESI_BAO=False, other_marginalizations_option=False):
	...
	ell_v = np.arange(ell_min, ell_max, 1)
...
### extract part of the Fisher matrix
def Fisher_submatrix(F, params_full, params_red):

	# match indices into the full Fisher matrix
	common_inds = []
	for p in range(len(params_red)):
		if not params_red[p] == 'None':
			common_inds.append( params_full.index( params_red[p] ))

	# grab the submatrix of interest from the full Fisher matrix
	F_red = np.zeros(( len(params_red), len(params_red) ))
	F_red = F[np.ix_(common_inds, common_inds)]
	#for i in range(len(common_inds)):
	#	for j in range(len(common_inds)):
	#		F_red[i,j] = F[common_inds[i], common_inds[j]]
	return F_red
		...
		for ch2 in range(nch):
			...
			if not key in Cls.keys():
				...
				if ch1 == ch2 :
					...
					if key == 'BuBu':
							Cov[ ch1 , ch2 , : ] += Cls['BB_delens'][ell_min-2:ell_max-1]
				else:
					Cov[ ch1 , ch2 , : ] = Cls[key][ell_min-2:ell_max-1]
